...
#include <jsk_recognition_msgs/PolygonArray.h> // Suqin
#include <ros/ros.h> // Suqin
...
vector_t FootPlacementConstraint::getValue(
  ...
  // read polygon message
  boost::shared_ptr<jsk_recognition_msgs::PolygonArray const> msg;
  msg = ros::topic::waitForMessage<jsk_recognition_msgs::PolygonArray>("polygon", ros::Duration(0.1));

  if (msg) {
    std::cout << "read topic /polygon succeed." << std::endl;
  }
